	...
	Convey("Moving Average Processor tests", t, func() {
		...
		Convey("Moving average for float32 data", func() {
			...
			for i, _ := range metrics {
				...
				rand.Seed(time.Now().UTC().UnixNano())
				data := randInt(65, 90)
				metrics[i] = *plugin.NewPluginMetricType([]string{"foo", "bar"}, time.Now(), "", nil, nil, float32(data))
			}
			var buf bytes.Buffer
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
		...
		Convey("Moving average for float64 data", func() {
			for i, _ := range metrics {
				...
				rand.Seed(time.Now().UTC().UnixNano())
				data := randInt(65, 90)
				metrics[i] = *plugin.NewPluginMetricType([]string{"foo", "bar"}, time.Now(), "", nil, nil, float64(data))
			}
			var buf bytes.Buffer
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
		...
		Convey("Moving average for uint32 data", func() {
			for i, _ := range metrics {
				...
				rand.Seed(time.Now().UTC().UnixNano())
				data := randInt(65, 90)
				metrics[i] = *plugin.NewPluginMetricType([]string{"foo", "bar"}, time.Now(), "", nil, nil, uint32(data))
			}
			var buf bytes.Buffer
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
		...
		Convey("Moving average for uint64 data", func() {
			for i, _ := range metrics {
				...
				rand.Seed(time.Now().UTC().UnixNano())
				data := randInt(65, 90)
				metrics[i] = *plugin.NewPluginMetricType([]string{"foo", "bar"}, time.Now(), "", nil, nil, uint64(data))
			}
			var buf bytes.Buffer
			enc := gob.NewEncoder(&buf)
			enc.Encode(metrics)
